#!/usr/bin/env python3
"""End-to-end CLI runner for the Camp Report Intelligence Engine.

Usage::

    python test_intelligence.py ./camp_report_json
    python test_intelligence.py ./camp_report_json -o ./output.json
    python test_intelligence.py ./camp_report_json --no-print-json

Loads a Camp Report JSON file, calls ``enrich_camp_report_with_intelligence``,
validates structure preservation, prints a summary, and writes an enriched JSON
file (never overwrites input).
...
from modules.reports.camp_report_intelligence import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "camp_report_json",
        type=Path,
        help="Path to input Camp Report JSON (e.g. ./camp_report_json)",
    )
    parser.add_argument(
        "-o",
        "--output",
