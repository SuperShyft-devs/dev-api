"""Load blood reports from Healthians for all engagements.

Same as ``db.jobs.load_blood_reports`` but includes every engagement status,
not only running.

Entrypoint: ``python -m db.jobs.all_engagements.load_blood_reports --yes``
"""
...
async def run_load(
    *,
    ...
    print("Loading eligible blood report participants for all engagements...", flush=True)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Load blood reports from Healthians for all engagements."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"\nLoad blood reports for all engagements ({mode}):\n"
