"""Tests for load-blood-reports participant endpoint."""
...
@pytest.mark.asyncio
async def test_load_blood_reports_route_requires_auth(async_client):
    response = await async_client.post(
        "/engagements/1/participants/load-blood-reports",
        json={"user_ids": [1]},
    )
...
@pytest.mark.asyncio
async def test_load_blood_reports_route_rejects_unknown_participants(async_client, test_db_session):
    engagement_id = 88201
    admin_id = 88201
    await _seed_employee(test_db_session, user_id=admin_id, employee_id=admin_id)
    await _seed_engagement(test_db_session, engagement_id=engagement_id, type_code="load_blood_reject")

    response = await async_client.post(
        f"/engagements/{engagement_id}/participants/load-blood-reports",
        headers=_auth_header(admin_id),
        json={"user_ids": [99988201]},
    )
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_load_blood_reports_route_calls_job_without_notifications(
    async_client,
    test_db_session,
    monkeypatch,
):
    engagement_id = 88202
    admin_id = 88202
    participant_id = 88203
    await _seed_employee(test_db_session, user_id=admin_id, employee_id=admin_id)
    await _seed_engagement(test_db_session, engagement_id=engagement_id, type_code="load_blood_ok")
    test_db_session.add(
        User(
    ...
    test_db_session.add(
        EngagementParticipant(
            engagement_participant_id=88202,
            engagement_id=engagement_id,
            user_id=participant_id,
    ...
    monkeypatch.setattr(
        "modules.notifications.load_blood_reports.load_blood_reports",
