"""Shared console actor: employee (admin) or partner (phlebo / organization_manager)."""
...
async def get_console_actor(
    request: Request,
    db: AsyncSession = Depends(get_db),
    credentials: HTTPAuthorizationCredentials | None = Depends(_http_bearer),
    employee_service: EmployeeService = Depends(get_employee_service),
) -> ConsoleActor:
    """Accept employee JWT (admin) or partner JWT (phlebo / organization_manager)."""
    ...
    if typ == "partner":
        ...
        return ConsoleActor(partner=partner)
    ...
    if typ == "employee":
        ...
        return ConsoleActor(employee=employee)
