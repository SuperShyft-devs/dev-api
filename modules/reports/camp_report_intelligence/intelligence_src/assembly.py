...
# Request aliases (engine ids, camp keys, nested metabolic/positives ids).
...
def resolve_intelligence_section(section: str) -> str:
    """Map a request ``section`` to a camp report section key.

    Accepts engine ids (``sleep``) and camp keys (``distribution_by_sleeping_hours``).
    Raises ``ValueError`` when the section is not intelligence-backed.
    """
...
def generate_camp_section_intelligence(report: dict, section: str) -> tuple[str, Any]:
    """Generate frontend intelligence for one camp/engine section only.

    Returns ``(camp_section_key, public_intelligence)``. Does not rewrite
    other sections. Raises ``ValueError`` for unknown sections.
    """
    if not isinstance(report, dict):
        raise TypeError("report must be a dict")
...
def enrich_camp_report_section_with_intelligence(report: dict, section: str) -> dict:
    """Deep-copy ``report`` and attach intelligence for a single section."""
    camp_key, intelligence = generate_camp_section_intelligence(report, section)
    enriched: Dict[str, Any] = copy.deepcopy(report)
    _attach_intelligence(enriched, camp_key, intelligence)
    return enriched
