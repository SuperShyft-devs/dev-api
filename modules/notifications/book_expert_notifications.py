"""Dispatch book-expert notifications for participants with unbooked consultations."""
...
async def dispatch_book_expert_notifications(
    db: AsyncSession,
    *,
    ...
    """Notify participants with camp tomorrow who have not booked offered consultations."""
    engagement_date = tomorrow_in_ist(as_of=as_of)
    participants = await engagements_repository.list_participants_for_book_expert(
        db,
    ...
    for user_id, engagement_id, service_keys in participants:
        ...
        try:
            ...
            logger.warning(
                "Book expert notification dispatch failed: service_keys=%s user_id=%s engagement_id=%s: %s",
                ",".join(service_keys),
                user_id,
                engagement_id,
                str(exc),
