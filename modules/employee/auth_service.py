"""Employee OTP authentication service."""
...
from modules.employee.models import Employee, EmployeeAuthOtpSession, EmployeeAuthToken
from modules.employee.repository import EmployeeRepository
from modules.notifications.service import NotificationsService


@dataclass(frozen=True)
class EmployeeTokenPair:
    access_token: str
    refresh_token: str


@dataclass(frozen=True)
class EmployeeOtpDelivery:
    employee_id: int
    name: str
    phone: str | None
    email: str | None
...
class EmployeeAuthService:
    """Employee OTP auth — mirrors partner AuthService patterns."""
    ...
    def _issue_access_token(self, employee_id: int) -> str:
        return create_access_token(employee_id, "employee")
    ...
    async def _resolve_employee_by_phone(
    ...
    async def _resolve_employee_by_email(self, db: AsyncSession, email: str) -> Optional[Employee]:
        return await self._repository.get_by_email(db, email)

    async def _issue_refresh_token(self, db: AsyncSession, employee_id: int) -> str:
        now = datetime.now(timezone.utc)
        expires_at = now + timedelta(days=settings.JWT_REFRESH_TOKEN_EXPIRE_DAYS)
        token_record = EmployeeAuthToken(
    ...
    async def send_otp(
        ...
        *,
        ...
        user_agent: str,
        endpoint: str,
    ) -> tuple[int, EmployeeOtpDelivery | None]:
        skip_send = False
        service_key: str | None = None

        if phone is not None:
            phone_candidates, skip_send = self._split_send_otp_phone(phone)
            employee = await self._resolve_employee_by_phone(db, phone_candidates)
            if not skip_send:
                service_key = settings.OTP_PHONE_SERVICE_KEY
        else:
            employee = await self._resolve_employee_by_email(db, email or "")
            if not skip_send:
        ...
        if (employee.status or "").lower() != "active":
            raise AppError(status_code=403, error_code="FORBIDDEN", message="Employee is not active")
        ...
        session = EmployeeAuthOtpSession(
        ...
        await self._repository.delete_all_otp_sessions_for_employee(db, employee.employee_id)
        created = await self._repository.create_otp_session(db, session)

        delivery: EmployeeOtpDelivery | None = None
        if not skip_send and service_key:
            delivery = EmployeeOtpDelivery(
                employee_id=employee.employee_id,
    ...
    async def deliver_otp(self, db: AsyncSession, *, delivery: EmployeeOtpDelivery) -> None:
    ...
    async def verify_otp(
        ...
        *,
        ...
        user_agent: str,
        endpoint: str,
    ) -> tuple[Employee, EmployeeTokenPair]:
        if phone is not None:
        ...
        if employee is None or (employee.status or "").lower() != "active":
            raise AppError(status_code=401, error_code="AUTH_FAILED", message="Authentication failed")

        session = await self._repository.get_latest_otp_session(db, employee.employee_id)
        if session is None:
            raise AppError(
                status_code=401,
        ...
        refresh_token = await self._issue_refresh_token(db, employee.employee_id)
        access_token = self._issue_access_token(employee.employee_id)

        await self._audit_service.log_event(
            db,
            action="EMPLOYEE_AUTH_LOGIN",
            endpoint=endpoint,
            ip_address=ip_address,
            user_agent=user_agent,
            user_id=None,
        ...
        return employee, EmployeeTokenPair(access_token=access_token, refresh_token=refresh_token)
    ...
    async def refresh_tokens(
        ...
        *,
        ...
        employee = await self._repository.get_by_id(db, record.employee_id)
        if employee is None or (employee.status or "").lower() != "active":
            raise AppError(status_code=401, error_code="AUTH_FAILED", message="Authentication failed")
        ...
        return employee, EmployeeTokenPair(
            access_token=self._issue_access_token(employee.employee_id),
